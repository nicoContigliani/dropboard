...
export default async function handler(
    ...
    // PUT - Actualizar un upload
        ...
        try {
            ...
            // CORRECCIÓN: Usar returnDocument: 'after' para obtener el documento actualizado
            const result = await db.collection('uploads').findOneAndUpdate(
                {
                    _id: new ObjectId(id),
                    'owner.id': decodedToken.userId
                },
    ...
    // DELETE - Eliminar un upload
        ...
        try {
            ...
            const result = await db.collection('uploads').findOneAndDelete({
                _id: new ObjectId(id),
                'owner.id': decodedToken.userId
            });
